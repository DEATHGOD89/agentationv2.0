...
function getReactDispatcher(): {
  ...
  // Try global React if exposed
  const globalReact = (window as any).React;
  if (globalReact) {
    const r19 = globalReact.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
    if (r19 && "H" in r19) {
      return {
        get: () => r19.H,
    ...
    const r18 = globalReact.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    if (r18 && r18.ReactCurrentDispatcher) {
      return {
        get: () => r18.ReactCurrentDispatcher.current,
        set: (d: unknown) => { r18.ReactCurrentDispatcher.current = d; },
      };
    }
  }

  // Fallback to React DevTools Global Hook
  const hook = (window as any).__REACT_DEVTOOLS_GLOBAL_HOOK__;
  if (hook && hook.renderers) {
    const renderers = Array.from(hook.renderers.values()) as any[];
    for (const renderer of renderers) {
      if (renderer.currentDispatcherRef) {
        return {
          get: () => renderer.currentDispatcherRef.current,
          set: (d: unknown) => { renderer.currentDispatcherRef.current = d; }
        };
      }
    }
  }
