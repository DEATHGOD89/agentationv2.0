...
// ============================================================================
// SCAN COMMAND - Universal project scanner
// ============================================================================

async function runScan(scanPath: string) {
  const { scanProject, generateMarkdown, generateJson } = await import("./scanner/index.js");
  ...
  console.error(`Scanning project at ${resolvedPath}...`);
...
// ============================================================================
// INIT SCAN COMMAND - Scan and create agentation config
// ============================================================================

async function runInitScan(scanPath?: string) {
  const { scanProject, generateJson } = await import("./scanner/index.js");
  ...
  try {
    const result = await scanProject(resolvedPath);
    const configPath = path.join(resolvedPath, "agentation.config.json");
    ...
    console.log(`✓ Agentation config written to ${configPath}`);
  } catch (err) {
    console.error("Scan failed:", err);
    process.exit(1);
  }
...
if (command === "init") {
  // Check if second arg is a path (for init scan) or no second arg (wizard)
  const secondArg = process.argv[3];
  if (secondArg && secondArg !== "--json" && !secondArg.startsWith("-")) {
    runInitScan(secondArg).catch((err) => {
      console.error("Init scan failed:", err);
      process.exit(1);
    });
  } else {
    runInit().catch((err) => {
...
Usage:
  agentation-mcp init [path]             Interactive setup wizard or scan project & create config
  agentation-mcp scan <path>             Scan a project and output results
...
Scan Options:
  --json             Output scan results as JSON (default: markdown)

Commands:
  init      Interactive setup wizard. With a path argument, scans the project and
            creates an agentation.config.json file.

  scan      Scans a project directory and outputs a detailed report including
            languages, framework, structure, dependencies, entry points, and more.
...
Examples:
  ...
  agentation-mcp init /path/to/project        Scan and create agentation config
  agentation-mcp scan .                       Scan current project
  agentation-mcp scan /path/to/project --json  Scan as JSON
